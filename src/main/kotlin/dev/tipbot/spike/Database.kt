    ...
    fun migrate(dataSource: DataSource) {
        dataSource.connection.use { conn ->
            conn.createStatement().use { st ->
                ...
                // Speed up the flood guard query: countLivePending(tipper_chat_id, status, expires_at)
                st.executeUpdate("CREATE INDEX IF NOT EXISTS idx_tips_tipper_pending ON tips(tipper_chat_id, status, expires_at)")
            }
